...
def scan():
    ...
    global journeyOn
    global eyeTotalCounter  
    global yawnTotalCounter
    ...
    while True:
        ...
        journeyOn = cords1.child("isJourneyOn").get().val()
...
eyeTotalCounter = m.ceil(eyeTotalCounter)
yawnTotalCounter = m.ceil(yawnTotalCounter) 

answer = str(eyeTotalCounter) +  ' ' +  str(yawnTotalCounter)

 
f = open( 'eyeYawnOutput.txt', 'w' )
f.write(answer)
f.close()

print("eye total counter :: " + str(eyeTotalCounter) + '\n\n')
print("yawn total counter :: " + str(yawnTotalCounter) + '\n\n')